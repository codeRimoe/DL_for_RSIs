...
#        ` IMGSize: the WIDTH, HEIGHT, BAND of image: [WIDTH, HEIGHT, BAND]
...
class SSM:
    ...
    def load_file(self, name):
        h5f = h5py.File(name + '_img','r')
        for i in h5f:
            self.img[i] = h5f[i][:]
        h5f.close()
        
        h5f = h5py.File(name + '_train','r')
        for i in h5f:
            self.train[i] = h5f[i][:]
        h5f.close()

        h5f = h5py.File(name + '_candidate','r')
        for i in h5f:
            self.candidate[i] = h5f[i][:]
        h5f.close()

        h5f = h5py.File(name + '_test','r')
        for i in h5f:
            self.test[i] = h5f[i][:]
        h5f.close()
